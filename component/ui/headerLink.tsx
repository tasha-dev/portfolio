...
export default function HeaderLink({children, link}:headerLinkType):ReactNode {
    ...
    return (
        <Link href={link} className={'group relative h-[40px]'}>
            <span className={'text-[24px] font-light dark:text-white text-black dark:hover:text-black hover:text-white relative z-10 transition-all duration-500'}>{children}</span>
